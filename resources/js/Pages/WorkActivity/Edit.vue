...
const user_form_work_activity = useForm({
    ...
    start_time: parse(props.work_activity?.start_time ?? '00:00:00', "HH:mm:ss", new Date(props.work_activity.work_event.start)),
    end_time: parse(props.work_activity?.end_time ?? '00:00:00', "HH:mm:ss", new Date(props.work_activity.work_event.end)),
    duration_hours: props.work_activity?.duration_hours ?? 0.0,
    notes: props.work_activity?.notes ?? '',
    tags: props.work_activity?.tags ?? [],
    status: props.work_activity?.status ?? 'pending',
    work_day: {
        date: new Date(props.work_activity.work_day.date),
        hourly_rate: props.work_activity.work_day?.hourly_rate ?? 0,
        total_events: props.work_activity.work_day?.total_events ?? 0,
        total_hours: props.work_activity.work_day?.total_hours ?? 0,
        amount: props.work_activity.work_day?.amount ?? 0,
    ...
    work_event: {
        editable: props.work_activity?.work_event?.editable ?? false,
        title: props.work_activity?.work_event?.title ?? '',
        allDay: props.work_activity?.work_event?.allDay ?? false,
        ...
        overlap: props.work_activity?.work_event?.overlap ?? false,
        editable: props.work_activity?.work_event?.editable ?? false,
        startEditable: props.work_activity?.work_event?.startEditable ?? false,
        durationEditable: props.work_activity?.work_event?.durationEditable ?? false,
        display: props.work_activity?.work_event?.display ?? 'auto',
    }
});
